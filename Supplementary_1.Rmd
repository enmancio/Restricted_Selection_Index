---
title: "Compute restriction selection index with simulated data"
output:
...
```{r, echo=TRUE}
setwd("~/Supplementary_codes/JDS")
source("JDS_function.R") # load all built-in function
pkg = c("AlphaSimR","MASS","ggplot2","pedigreemm")
libraries(pkg)
set.seed(123)
```
...
### Scenario 5
For computational reasons  we reduced the population size and for the need to have a pedigree to accurately estimate the genetic value of animals, before selection, we perform 3 generation of random mating. 
Selection intensity was incresd for a better visual appraisal og the plots.
...
for (i in 1:3){
    candidate = selectInd(pop,nInd=pop@nInd*1/4, use = "rand",sex = "B")
